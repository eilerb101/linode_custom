...
cpu=$(get_config "cpu")
virtualdisk=$(( $(get_config "virtual_disk") * 1024))
...
required_vars=("token" "region" "linode_type" "memory" "network" "virtualdisk")
...
site_type=$(echo "$regions_response" | jq -r --arg REGION "$region" \
  '.data[] | select(.id==$REGION) | .site_type')
if [[ "$site_type" == "core" ]]; then
    DISTRO="linode/alpine3.22"
    YAML="template.yaml"
    alpine_disk_size=520

elif [[ "$site_type" == "distributed" ]]; then
    DISTRO="linode/debian12"
    YAML="template-deb.yaml"
    alpine_disk_size=1820
    linode_type="edge"
    echo "Distributed Sites require Dedicated Instances ONLY, enforcing instance type"
else
    echo "Unknown site_type: $site_type"
    exit 1
fi
...
if [[ "$site_type" == "distributed" ]]; then
    log_info "Filtering for EDGE instance types for distributed site..."
    types_response=$(echo "$types_response" | jq '.data |= map(select(.id | test("-edge-")))')
fi
...
#alpine_disk_size is now determined during swing distro determination based on region site type above
#raw_disk_size=$((type_disk - alpine_disk_size))
raw_disk_size=$virtualdisk
#raw_disk_size=10240
...
#BEFORE BOOT - READ IN TEMPLATE AND CREATE BASE64 ENC YAML USER DATA

cloud_init_template=$(cat "$YAML")

cloud_init_yaml="${cloud_init_template//##TOKEN##/$token}"
cloud_init_yaml="${cloud_init_yaml//##REGION##/$region}"
cloud_init_yaml="${cloud_init_yaml//##NETWORK##/$network}"
cloud_init_yaml="${cloud_init_yaml//##BUCKET_NAME##/$bucket_name}"
cloud_init_yaml="${cloud_init_yaml//##IMAGE_NAME##/$image_name}"
...
cloud_init_yaml="${cloud_init_yaml//##SS_PASSTHROUGH_PAYLOAD##/$escaped_payload}"
...
log_info "=== Creating instance... API response= $instance_response ==="
echo "=== Creating instance... API response= $instance_response ==="
...
Updated YAML: $cloud_init_yaml
Base64 Encoded YAML: $cloud_init_base64
CloudInit API Response: $update_response 
REQUEST PARAMETERS:
