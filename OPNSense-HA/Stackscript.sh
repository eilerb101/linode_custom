...
# <UDF name="instance_role" label="Instance Role (active or standby)" />
# <UDF name="shared_ipv4" label="Shared IPv4 Address" />
...
INSTANCE_ROLE="${INSTANCE_ROLE:-$instance_role}"
SHARED_IPV4="${SHARED_IPV4:-$shared_ipv4}"
...
# Extract Alpine and OPNSense config IDs based on label matches
ALPINE_CONFIG=$(echo "$configs" | jq -r '.data[] | select(.label | test("Alpine"; "i")) | .id' | head -n1)
log "Alpine Config: $ALPINE_CONFIG"
OPNSENSE_CONFIG=$(echo "$configs" | jq -r '.data[] | select(.label | test("System"; "i")) | .id' | head -n1)
log "OPNSense Config: $OPNSENSE_CONFIG"
...
if [ -z "$OPNSENSE_CONFIG" ] || [ "$OPNSENSE_CONFIG" = "null" ]; then
...
log "Discovered OPNSense Config ID: $OPNSENSE_CONFIG"
...
# Boot OPNSense configuration with retry
log "Booting OPNSense configuration (ID: $OPNSENSE_CONFIG)..."
...
# Boot OPNSense configuration with verification
#log "Rebooting to OPNSense configuration (ID: $OPNSENSE_CONFIG)..."
#boot_response=$(api_call "POST" "${API_BASE}/linode/instances/${INSTANCE_ID}/reboot")
#if [ $? -ne 0 ]; then
#    log "ERROR: Failed to initiate reboot after retries"
#    log "Response: $boot_response"
#    log "Attempting manual power cycle..."
    # Try shutdown then boot as fallback
#    api_call "POST" "${API_BASE}/linode/instances/${INSTANCE_ID}/shutdown"
#    sleep 5
#    api_call "POST" "${API_BASE}/linode/instances/${INSTANCE_ID}/boot" "{\"config_id\": $OPNSENSE_CONFIG}"
#    exit 1
#else
#    log "Reboot request accepted by API"
#    log "OPNSense configuration boot initiated"
#fi
log "OPNSense configuration boot initiated"
log "=== Image Conversion Process Complete ==="
log "The system will now reboot into the OPNSense Platform"
